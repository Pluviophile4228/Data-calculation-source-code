...
class FaultTreeAnalyzer:
    ...
    def __init__(self, logic_relations, failure_intervals):
        ...
        Parameters:
            logic_relations: List of fault tree logic relations, formatted as [(parent node, child node, logic gate), ...]
            failure_intervals: Dictionary of basic event failure probability intervals, formatted as {event: (lower bound, upper bound), ...}
        """
        # Store original data
    ...
    def _convert_intervals_to_float(self, intervals):
        """Convert probability intervals in string format to floats"""
        return {
    ...
    def _validate_structure(self):
        ...
        for node in self.non_basic_events:
            if node not in self.gate_types:
                raise ValueError(f"Non-basic event '{node}' has no defined logic gate type. Please check logic relations.")
    ...
    def compute_node_probability(self, node, prob_dict):
        ...
        Parameters:
            ...
            prob_dict: Basic event probability dictionary {event: probability value}

        Returns:
            Failure probability of the node
        """
        # Basic events return probability directly
    ...
    def analyze(self, n_simulations=10000, plot_cdf=True):
        """
        Complete analysis process: simulation + calculate 95% confidence level point estimate + visualization
        ...
        Returns:
            Top event failure probability at 95% confidence level (point estimate, taking upper confidence interval)
        """
        ...
        point_estimate = ci_upper  # Take upper confidence interval as conservative estimate
    ...
    def _plot_cdf(self, top_probs, point_estimate, ci_lower, ci_upper):
        """Plot cumulative distribution function (CDF) of top event failure probability"""
...
def main():
    """Main function: define parameters and execute analysis"""
    # 1. Define fault tree parameters
    failure_intervals = {
        "COM1 failure": ["3.400e-06", "8.111e-05"],
        "COM2 failure": ["3.400e-06", "8.111e-05"],
        "MON1 failure": ["3.000e-06", "7.157e-05"],
        "MON2 failure": ["3.000e-06", "7.157e-05"],
